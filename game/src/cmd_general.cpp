...
ACMD(do_restart)
{
	...
	if (subcmd != SCMD_RESTART_TOWN && (!ch->GetWarMap() || ch->GetWarMap()->GetType() == GUILD_WAR_TYPE_FLAG))
	{
		if (!test_server)
		{
			...
			if (iTimeToDead > 178)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¾ÆÁ÷ Àç½ÃÀÛ ÇÒ ¼ö ¾ø½À´Ï´Ù. (%dÃÊ ³²À½)"), iTimeToDead - 178);
				return;
			}
		}
	}
		...
		while ((row = mysql_fetch_row(pRes->pSQLResult)) != NULL)
		{
			...
			if (vnum == 1)
			{
				...
				if (GOLD_MAX <= nTotalMoney)
				{
					...
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("20¾ï ³ÉÀ» ÃÊ°úÇÏ¿© »óÁ¡À» ¿­¼ö°¡ ¾ø½À´Ï´Ù"));
					return true;
				}
#endif
#ifdef FULL_YANG_OWN
...
ACMD(do_set_name_shop)
{
	...
	if (m_stShopSign.length()<1) {
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ºñ¼Ó¾î³ª Àº¾î°¡ Æ÷ÇÔµÈ »óÁ¡ ÀÌ¸§À¸·Î »óÁ¡À» ¿­ ¼ö ¾ø½À´Ï´Ù."));
		return;
	}
	...
	*/
...
#endif
	{
		...
		if (item)
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT( "À¯·áÈ­ ¾ÆÀÌÅÛÀº °³ÀÎ»óÁ¡¿¡¼­ ÆÇ¸ÅÇÒ ¼ö ¾ø½À´Ï´Ù."));
				do_shop_refresh_items(ch,args[1].c_str(),0,0);
				return;
			}
			...
			if (item->IsEquipped())
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT( "ÀåºñÁßÀÎ ¾ÆÀÌÅÛÀº °³ÀÎ»óÁ¡¿¡¼­ ÆÇ¸ÅÇÒ ¼ö ¾ø½À´Ï´Ù."));
				do_shop_refresh_items(ch,args[1].c_str(),0,0);
				return;
			}
			...
			if (item->isLocked())
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT( "»ç¿ëÁßÀÎ ¾ÆÀÌÅÛÀº °³ÀÎ»óÁ¡¿¡¼­ ÆÇ¸ÅÇÒ ¼ö ¾ø½À´Ï´Ù."));
				do_shop_refresh_items(ch,args[1].c_str(),0,0);
				return;
			}
			if (item->GetOwner() != ch)
...
ACMD(do_lang3)
{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "Jazyk: Ceština.");
}
ACMD(do_lang4)
{
		char query[1024];
...
ACMD(do_lang7)
{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "Langue: Français.");
}
ACMD(do_lang8)
{
		char query[1024];
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "Glóssa: Elliniká.");
}
ACMD(do_lang9)
{
		char query[1024];
...
ACMD(do_lang11)
{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "Jêzyk: Polski.");
}
ACMD(do_lang12)
{
		char query[1024];
...
ACMD(do_lang13)
{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "Limba: Românã.");
}
ACMD(do_lang14)
{
		char query[1024];
...
ACMD(do_lang15)
{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "Dil: Türkçe.");
}
