...
bool CMobManager::LoadGroupGroup(const char * c_pszFileName, bool isReloading)
{
	...
	if (isReloading)
	{
		for (auto it = m_map_pkMobGroupGroup.begin(); it != m_map_pkMobGroupGroup.end(); it++)
			M2_DELETE(it->second);
		m_map_pkMobGroupGroup.clear();
		for (auto it = tempLoader.begin(); it != tempLoader.end(); it++)
		{
...
bool CMobManager::LoadGroup(const char * c_pszFileName, bool isReloading)
{
	...
	if (isReloading)
	{
		for (auto it = m_map_pkMobGroup.begin(); it != m_map_pkMobGroup.end(); it++)
			M2_DELETE(it->second);
