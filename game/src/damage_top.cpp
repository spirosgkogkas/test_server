...
TDamage CDamageTop::GetDamage(LPCHARACTER pkChar, BYTE bType)
{
	...
	for(int i = 0; i < vecDamageTop.size(); i++)
	{
...
void CDamageTop::UpdateDamageExtern(LPDESC d, const char* c_pData)
{
	...
	if(!vecDamageTop.empty())
	{
		for (int i = 0; i < vecDamageTop.size(); i++)
		{
			...
			{
				bContain = true;
				thecore_memcpy(&vecDamageTop[i], &p->damageT, sizeof(vecDamageTop[i]));
				break;
			}
		}
	}
...
void CDamageTop::SendGGUpdateDamage(TDamage damageT)
{
	...
	thecore_memcpy(&p2pPacket.damageT, &damageT, sizeof(p2pPacket.damageT));
...
{
	if (NULL == pkVictim || NULL == pkAttacker)
		return;
	...
	if(damageInfo.dwDamageValue != 0)
	{
		if(damageInfo.dwDamageValue < dam)
		{
...
struct FDamageCompare : public std::binary_function<TDamage, TDamage, bool>
{
